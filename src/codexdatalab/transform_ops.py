...
def init_transform(workspace: Workspace, dataset_id: str, name: str, *, why: str = "") -> Path:
    ...
    transform_path = workspace.root / "transforms" / filename
...
output_path = WORKSPACE_ROOT / \"data\" / \"{dataset_id}_cleaned.csv\"
...
def run_transform(
    ...
    *,
    ...
    before = {p.resolve() for p in (workspace.root / "data").iterdir() if p.is_file()}
    ...
    after = {p.resolve() for p in (workspace.root / "data").iterdir() if p.is_file()}
    ...
    workspace.commit("Run transform", paths=["data", ".codexdatalab/manifest.json", ".codexdatalab/lineage.json"])
